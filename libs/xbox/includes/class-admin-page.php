...
class AdminPage extends XboxCore {
    ...
    */
    private function hooks(){
        ...
        add_action( 'xbox_after_save_fields_admin-page', array( $this, 'after_save_fields' ), 10, 3 );
    }
    ...
    */
    public function after_save_fields( $data, $object_id, $updated_fields = array() ){
        ...
        if( ! $this->import ){
            //Save fields(Fix for WPML String Translations)
            $options = (array) get_option( $this->object_id );
            if( $options && $this->fields_for_save ){
                $new_options = array_replace_recursive( $options, $this->fields_for_save );
                update_option( $this->object_id, $new_options );
            }
        }
